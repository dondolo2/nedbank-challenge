...
This document is the authoritative specification of the interface between your Docker image and the automated evaluation system. The evaluation system is built to this contract. If the contract and your submission conflict, your submission will be scored as a failure.
...
The evaluation system mounts a single host directory to `/data` inside the container. All input and output paths live under `/data/`.

### Input (read-only)

| Mount path | Contents |
|---|---|
| `/data/input/accounts.csv` | Fintech account data (CSV) |
| `/data/input/transactions.jsonl` | Fintech transaction data (JSONL) |
| `/data/input/customers.csv` | Bank customer data (CSV) |
| `/data/config/pipeline_config.yaml` | Runtime configuration; the evaluation system may inject overrides |

Your pipeline must not write to any input path. These mounts are enforced read-only by the evaluation system (`--read-only` with explicit output exceptions).

### Output (read-write)

| Mount path | Contents |
|---|---|
| `/data/output/bronze/` | Bronze layer Delta tables |
| `/data/output/silver/` | Silver layer Delta tables |
| `/data/output/gold/` | Gold layer Delta tables (scored) |
...
The evaluation system invokes your container as follows:
...
The evaluation system wraps the `docker run` invocation in a 30-minute (`1800s`) timeout. If your container has not exited by T+30 minutes, it will be killed.

---

## 5. Exit Codes

The evaluation system reads the exit code before attempting to process any outputs.
