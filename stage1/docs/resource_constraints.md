...
Your pipeline runs inside a Docker container with hard resource limits enforced by the evaluation system. These are not soft suggestions. Exceeding any limit produces a non-zero exit code, which results in zero correctness points for that stage.
...
**Why this limit exists:** The data may approach or exceed the 2 GB RAM allocation depending on encoding overhead. This constraint makes streaming, partitioned, or lazy-evaluation processing **mandatory**, not merely recommended. Pipelines must process data incrementally (e.g., chunked reads, Spark partitioned scans, or DuckDB out-of-core execution) to avoid immediate OOM termination.
...
- Pandas operations on small subsets of data — reading one table at a time, processing in chunks, writing before loading the next. Chunk-based reading is mandatory.
...
- `pandas.read_json()` on the full JSONL file. This will allocate several multiples of the file size in memory and OOM.
- `spark_df.toPandas()` on a large Spark DataFrame. This collects all data to the driver.
- Loading all three source tables into Pandas DataFrames simultaneously.
...
Run this in a second terminal while your pipeline executes. Headroom below the 2 GB ceiling reduces OOM risk on the scoring run.
...
**What it means:** Your pipeline must start, process all data, write all outputs, and exit with code `0` within 1800 seconds of the container starting. The evaluation system wraps the `docker run` invocation with a timeout; when the timer expires, the container is killed and receives exit code `124`.

**Why this limit exists:** The 30-minute limit is calibrated so that a well-optimised PySpark pipeline can process the dataset comfortably. The constraint distinguishes efficient pipelines from inefficient ones. Execution time is a scored dimension.
...
If this completes well under 30 minutes on the provided sample data (10,000 rows), profile your pipeline on the full provided data (1 million transactions) before submitting.
...
1. **Test with production-scale flags from the beginning.** Running `docker run -m 2g --cpus=2` locally on the provided data will surface memory and timing problems before you encounter them in the scorer.
...
3. **Read each source file once.** Cache a DataFrame if you need to reference it multiple times. Reading the same JSONL file twice costs significant wall-clock time.
...
5. **Monitor during development.** Use `docker stats` to track memory and CPU utilisation during local test runs. Headroom below the 2 GB ceiling reduces OOM risk on the scoring run.
