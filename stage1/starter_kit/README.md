...
Copy the data files from the challenge pack `data/` directory to a local
directory. The examples below use `/tmp/test-data`:

```bash
mkdir -p /tmp/test-data/input /tmp/test-data/config
cp ../data/accounts.csv         /tmp/test-data/input/accounts.csv
cp ../data/transactions.jsonl   /tmp/test-data/input/transactions.jsonl
cp ../data/customers.csv        /tmp/test-data/input/customers.csv
...
Your pipeline must exit with code 0 for the evaluation system to read your output.
