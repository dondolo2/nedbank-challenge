...
Pipelines that handle Stage 2 through configuration changes and targeted additions score higher on Maintainability than those requiring structural rewrites.
...
- The `dq_flag` column in `fact_transactions` must be set to the relevant issue code (listed above) or `NULL` for clean records. No other values are accepted by the evaluation system.
...
|---|---|---|---|
| `merchant_subcategory` | STRING | Yes | Sub-classification within `merchant_category`. Present in Stage 2 data onward. Absent from Stage 1 data. |
...
> **Note:** `fact_transactions` field count is **15** in Stage 2 (was 14 in Stage 1). The full inline schema is reproduced below for convenience.

| Position | Name | Type | Nullable | Description |
|---|---|---|---|---|
| 1 | `transaction_sk` | BIGINT | No | Surrogate key. Pipeline-generated, unique, non-null. |
| 2 | `transaction_id` | STRING | No | Natural key from `transactions.jsonl`. Deduplicated in Silver. |
| 3 | `account_sk` | BIGINT | No | FK to `dim_accounts.account_sk`. |
| 4 | `customer_sk` | BIGINT | No | FK to `dim_customers.customer_sk` via `account_sk`. |
| 5 | `transaction_date` | DATE | No | Standardised to ISO `YYYY-MM-DD`. |
| 6 | `transaction_timestamp` | TIMESTAMP | No | Combined `transaction_date` + `transaction_time`, ISO 8601. |
| 7 | `transaction_type` | STRING | No | Values: `DEBIT`, `CREDIT`, `FEE`, `REVERSAL`. |
| 8 | `merchant_category` | STRING | Yes | MCC-style code from source. Nullable where absent. |
| 9 | `merchant_subcategory` | STRING | Yes | **New in Stage 2.** Sub-classification within `merchant_category`. Null when absent or null in source. |
| 10 | `amount` | DECIMAL(18,2) | No | Numeric after type standardisation. Two decimal places. |
| 11 | `currency` | STRING | No | Must be standardised to `"ZAR"` regardless of source variant. |
| 12 | `channel` | STRING | No | Values: `POS`, `APP`, `ATM`, `EFT`, `USSD`, `INTERNAL`. |
| 13 | `province` | STRING | Yes | From `transactions.jsonl` `location.province`. |
| 14 | `dq_flag` | STRING | Yes | Pipeline-derived. Null for clean records, otherwise one of the six DQ issue codes. |
| 15 | `ingestion_timestamp` | TIMESTAMP | No | Pipeline-generated. When the record entered the Bronze layer. |
...
The report must conform to the schema and format defined in `dq_report_template.json`.

Add `"stage": "2"` as a top-level field in `dq_report.json` alongside `run_timestamp`.
...
The evaluation system cross-references `records_affected` values against internally computed expected counts. It also verifies that `handling_action` values in the report are consistent with your `dq_rules.yaml`.

The file must be present at exactly `/data/output/dq_report.json`. No other path is checked.

> **No new validation queries are added in Stage 2.** The Stage 1 validation queries continue to apply, plus the DQ report becomes a new automated check.
...
The evaluation system reads `config/dq_rules.yaml` and cross-references it against your `dq_report.json`. Inconsistencies between what the config declares and what the report records are flagged as correctness failures.

The intent is that a DQ rule can be changed — swapping `QUARANTINED` to `NORMALISED` for orphaned accounts, for example — by editing the config file, not by modifying pipeline code.
...
- Entry point: same `docker run` flags as Stage 1 — see Stage 1 `docker_interface_contract.md` §4

All Stage 1 validation queries must still pass on Stage 2 data with DQ handling applied.

---

*Stage 2 Specification Addendum — DE Track.*
