...
All 12 stream files are pre-staged in /data/stream/ at container start. Filename order matches chronological order. Each file contains between 50 and 500 transaction events. Files are named with the convention `stream_YYYYMMDD_HHMMSS_NNNN.jsonl` (e.g., `stream_20260320_143000_0001.jsonl`).
...
Your pipeline must poll the directory, detect new files as they arrive, and process them. See `stream_interface_spec.md` for the full technical specification of the streaming interface, file format, and polling contract.
...
Streaming events must be reflected in the Gold layer within 5 minutes of arrival (the SLA target).

The evaluation system checks `current_balances.updated_at` and `recent_transactions.updated_at` against the event timestamps in the processed stream files.
...
Stage 3 uses the **same base data files as Stage 2**. The input mounts at `/data/input/` contain the Stage 2 accounts, customers, and transactions — including all DQ injections from Stage 2.

This means:
- Your Stage 2 batch pipeline processes the same `accounts.csv`, `customers.csv`, and `transactions.jsonl` as in Stage 2
- The streaming events in `/data/stream/` represent **new** transactions that arrive after the batch data was generated
- Your streaming pipeline should update account balances and recent transactions based on the streaming events, using the batch-processed data as the baseline

The evaluation system mounts Stage 2 base data at `/data/input/`.

---

## 8. Batch Pipeline Must Still Work (unchanged)

Run the batch pipeline to completion before starting the streaming loop. The streaming pipeline depends on account data written by the batch pipeline.
...
All stream files are present in `/data/stream/` at container start. A polling loop is still a useful pattern for production-like code, but at evaluation time your loop will find all files immediately.

---

*Stage 3 Specification Addendum — DE Track.*
