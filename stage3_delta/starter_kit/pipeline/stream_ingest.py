...
Input paths:
  /data/stream/            — Directory of micro-batch JSONL files. All 12
                             stream files are present in /data/stream/ at
                             container start. Process them in filename order —
                             lexicographic ordering matches chronological. A
                             polling loop is still a useful pattern for
                             production-like code, but at evaluation time your
                             loop will find every file on the first iteration.
...
Requirements:
  - Process all files in /data/stream/ in filename order. (A polling loop is
    still acceptable; at evaluation time it will find every file on the first
    iteration.)
  - Process each file and merge results into the stream_gold tables.
  ...
  - SLA: updated_at must be within 300 seconds of the source event timestamp
    for full SLA compliance; 301–600 seconds is outside SLA.
...
See stream_interface_spec.md §4 and §5 for the full field-by-field specification
of current_balances and recent_transactions.

See stream_interface_spec.md §1 for the /data/stream/ mount details.
"""
