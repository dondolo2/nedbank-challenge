...
| **File naming** | `stream_YYYYMMDD_HHMMSS_NNNN.jsonl` — date-time prefix with sequential zero-padded suffix (e.g., `stream_20260320_143000_0001.jsonl`) |
...
| **Delivery cadence** | All 12 files present at container start. Filename order matches delivery order. |
| **Delivery mechanism** | All stream files are present in `/data/stream/` at container start. A polling loop is still a useful pattern for production-like code, but at evaluation time your loop will find all files immediately. |
...
| Latency | Outcome |
|---|---|
| ≤ 300 seconds | meets SLA |
| 301–600 seconds | outside SLA |
| > 600 seconds | outside SLA |
...
- No exactly-once delivery guarantee is assessed — the evaluation system does not inject duplicate stream files
